    ...
    <div className="space-y-4">
      ...
      {result && (
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className={`text-sm mt-1 ${result.success ? "text-green-800" : "text-red-800"}`}>{result.message}</p>
            {result.attendee && (
              <div className={`text-sm mt-2 space-y-1 ${result.success ? "text-green-800" : "text-red-800"}`}>
                <p>
          ...
          {result && (
            <button onClick={() => setResult(null)} className="text-foreground/40 hover:text-foreground">
              <X className="w-5 h-5" />
            </button>
          )}
        </Card>
      ...
      <div className="flex gap-2">
        <Button
          ...
          className={`flex-1 ${scanning ? "bg-red-600 hover:bg-red-700" : "bg-primary hover:bg-primary/90"} text-white`}
        >
      ...
      <Card className="p-4 bg-muted/50">
        ...
        <div className="flex gap-2">
          <Input
            ...
            disabled={isProcessing}
          />
          <Button onClick={handleManualCheckin} disabled={!manualTicketId.trim() || isProcessing} variant="outline">
            Check In
          </Button>
        </div>
      </Card>
